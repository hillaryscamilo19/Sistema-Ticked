...
export function RegisterForm() {
  ...
  const [phone_ext, setExtension] = useState("");
  ...
  const handleRegister = async () => {
    ...
    const payload = {
      fullname: fullname,
      email: email,
      phone_ext: parseInt(phone_ext),
      department_id: parseInt(departamentoSeleccionado),
      role: 1,
      username: username,
      password: password,
      status: false,
...
if (response.status === 201) {
  ...
  const errorData = await response.json(); // ✅ así obtienes el body de error
        ...
        if (errorData.detail && Array.isArray(errorData.detail)) {
          ...
          setError(
            errorData.message ||
              ...
              "Error al registrar. Verifica los campos."
          );
        }
      } else {
